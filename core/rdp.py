...
class RDPConnector:
    """Classe para gerenciar conexões RDP de forma síncrona"""
    
    @staticmethod
    def verificar_dependencias() -> bool:
        """Verifica se xfreerdp está disponível"""
        try:
            subprocess.run(["xfreerdp", "--help"], 
    ...
    @staticmethod
    def conectar_sync(host: str, usuario: str, senha: str, opcoes: Optional[Dict] = None) -> bool:
        """
        Conecta via RDP de forma síncrona
        
        Args:
            host: Endereço do servidor
            usuario: Nome do usuário
            senha: Senha do usuário
            opcoes: Opções de conexão
            
        Returns:
            True se conectou com sucesso
            
        Raises:
            RDPConnectionError: Em caso de erro na conexão
        """
        if not RDPConnector.verificar_dependencias():
            raise RDPConnectionError("xfreerdp não encontrado")
        
        opcoes = opcoes or {}
        
        # Criar comando
        cmd = RDPConnector._construir_comando_rdp(host, usuario, senha, opcoes)
        
        logger.info(f"Iniciando conexão RDP síncrona para {host}")
    ...
    @staticmethod
    def _construir_comando_rdp(host: str, usuario: str, senha: str, opcoes: Dict) -> list:
        """Constrói comando xfreerdp - versão estática"""
        cmd = [
            "xfreerdp",
            f"/u:{usuario}",
            f"/p:{senha}",
        ...
        # Aplicar opções
        if opcoes.get('clipboard', False):
            cmd.append("/clipboard")
            
        if opcoes.get('montar_home', False):
            home = obter_pasta_home()
            if os.path.exists(home):
                cmd.append(f"/drive:c,{home}")
        
        som = opcoes.get('som', 'local')
        if som == 'remoto':
            cmd.append("/audio-mode:1")
        elif som == 'ambos':
            cmd.append("/audio-mode:2")
        ...
        if opcoes.get('impressoras', False):
            cmd.append("/printer")
        
        if opcoes.get('multimonitor', False):
            cmd.extend(["/multimon", "/span"])
        
        resolucao = opcoes.get('resolucao', 'auto')
        if resolucao != 'auto':
            cmd.append(f"/size:{resolucao}")
        
        qualidade = opcoes.get('qualidade', 'broadband')
        if qualidade == 'modem':
            cmd.extend(["/bpp:8", "/compression-level:2"])
        elif qualidade == 'broadband':
            cmd.extend(["/bpp:16", "/compression-level:1"])
